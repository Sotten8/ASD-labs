...
def draw_arrow(canvas, x1, y1, x2, y2, radius=25, color="darkblue", width=2):
    dx, dy = x2 - x1, y2 - y1
    ...
    canvas.create_line(
        start_x, start_y, end_x, end_y, arrow=tk.LAST, width=width, fill=color
    )
...
def draw_arc(
    canvas, x1, y1, x2, y2, radius=25, directed=True, color="darkblue", width=2
):
    dx, dy = x2 - x1, y2 - y1
    dist = math.hypot(dx, dy)
    if dist == 0:
...
def draw_self_loop(canvas, x, y, directed=True, color="darkblue", width=2):
    ...
    if directed:
        canvas.create_line(
            *arrow_start, *arrow_end, width=width, fill=color, arrow=tk.LAST
        )
